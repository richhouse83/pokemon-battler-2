...
const pickPokemon = (player) => {
  return inquirer
    ...
    .then((movesDetails) => {
      player.currentPokemon.movesList = movesDetails;
      console.log("Available moves:");
      movesDetails.forEach((move) => {
        console.log(
          `Name: ${move.name}, Accuracy: ${move.accuracy}, Power: ${move.power}, Power Points: ${move.pp}`
        );
      });
...
const determineOutcome = (attacker, defender) => {
  ...
  let chanceOfSuccess = Math.floor(Math.random() * 105);
